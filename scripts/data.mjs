...
export const PROJECTS = [
  {
    ...
    headline: "AI-assisted neural-model experiments",
    accent: "#aeb8c4", light: "#5b6672", grad: "#1c2530",
    hint: "#dfe7ee", hintLight: "#5b6672",
    phase: "research", repo: "sepahead/engram",
    desc: "Builds and runs neural-model experiments with an AI agent and custom harness. Optional NCP controller. Public source pending.",
    stack: ["AI Agent", "NEST"],
    summary: "Engram builds and runs experiments with biophysical and functional neural models. An AI agent and custom harness track model sources, simulation inputs, and results. Its optional NCP controller exchanges observations and action proposals with separate applications. The tested implementation is in private Paper2Brain source. The public repository remains a placeholder pending source publication; it does not claim general paper reproduction or validated scientific results.",
    status: "Research-stage system; public source pending publication; no claim of general paper reproduction or validated scientific results.",
  ...
  },
  {
    ...
    headline: "Shared protocol for neural and simulation apps",
    ...
    summary: "Neuro-Cybernetic Protocol (NCP) defines typed messages for neural controllers, simulations, sensor monitors, and experiment capture. Applications select optional local peers through private process channels. A producer can expose several independently identified sensors through one endpoint. Selected installed sensor and neural compositions have bounded native evidence. Broader qualification and final v1 release gates remain open. Haldir uses the separate pinned v0.8 interface.",
    status: "Local v1 simulation candidate; selected installed compositions have bounded evidence; broader qualification and final release gates remain open.",
  ...
  },
  {
    ...
    headline: "World-model experiments for embodied agents",
    accent: "#a78bfa", light: "#7c3aed", grad: "#241a44",
    phase: "research", repo: "sepahead/prisoma",
    desc: "LeWM inference and search on CPU/MPS. Canonical CREBAIN sensor execution. Full forecast and restored-label experiments remain open.",
    stack: ["Rust", "Python"],
    summary: "Prisoma develops controlled experiments for action-conditioned world models and embodied agents. Pretrained LeWM inference and candidate search ran locally on CPU and MPS on an M4 Max using one frozen PushT input. A separate deterministic affine reference commits forecasts, selects and executes an action, and compares outcomes from independently restored branches. These tests establish software behavior within their declared inputs, not learned-model decision quality. LeWM's standardized candidates are distinct from raw environment commands. CREBAIN owns the selected 3D environment and sensors. Prisoma owns experiment design, comparisons, and evidence. Its optional Agent Bridge records accepted sensor commands and reconstructs their original NCP exchanges. Separate cameras retain their identities and timing. Learned raw-action execution, forecast comparisons, and independently restored outcome labels remain open for the complete embodied experiment. Rerun export supplies derived visualizations. The separate NCP transcript records exchanges for explicitly selected peers. PID is an optional diagnostic with separate validity requirements.",
  ...
  },
  {
    ...
    headline: "Standalone 3D simulation and sensors",
    accent: "#a1b39a", light: "#425c47", grad: "#15231e",
    stars: 20, repo: "sepahead/crebain",
    desc: "Standalone drone simulation with 3DGS/mesh RGB, microphone pressure, and thermal radiance. Optional NCP; sensor-fusion research.",
    stack: ["TS", "Rust", "Nix"],
    summary: "CREBAIN (Adaptive Response & Awareness System) provides standalone 3D drone simulation, native sensor observations, and sensor-fusion research. Its environment supports Gaussian-splat and mesh RGB views, microphone pressure samples, and thermal radiance. A selected one-drone force-control experiment passed branch comparisons with actual RGB, acoustic, and thermal output in a mesh scene. The separate city profile admits up to 256 drones; that limit does not establish tracking quality or physical realism. A separate local NCP scalar profile covers one through three entities. Optional NCP interfaces connect controllers, tampering monitors, and capture. CREBAIN runs without Engram, Galadriel, or Prisoma. Prisoma can record canonical sensor commands or capture selected exchanges. The complete forecasting and restored-label experiment remains open.",
    status: "Research prototype with tested bounded force control and native sensor branches; physical realism, larger scenarios, and ROS integration remain under qualification.",
    schemaType: "SoftwareSourceCode",
  ...
  },
  {
    ...
    headline: "Deterministic 3DGS asset conversion",
    accent: "#fb923c", light: "#c2410c", grad: "#2a1608",
    stars: 13, repo: "sepahead/melkor",
    desc: "Deterministic 3DGS conversion across PLY, SPZ and glTF, surfacing field provenance and numeric hazards. Source-only RC; no production binary.",
    stack: ["PLY/SPZ", "Inspection", "Provenance"],
    summary: "Melkor provides deterministic 3D Gaussian Splatting (3DGS) conversion across PLY, SPZ and glTF. Its inspection reports surface field provenance, bounds and numeric hazards without modifying source assets. The current release candidate is source-only; no production binary is currently supported.",
    status: "Current release candidate is source-only; no production binary is currently supported.",
    schemaType: "SoftwareSourceCode",
  ...
  },
  {
    ...
    headline: "Optional sensor-tampering monitor",
    accent: "#ef4444", light: "#dc2626", grad: "#2a0a0a",
    stars: 3, repo: "sepahead/galadriel",
    desc: "Optional sensor checks report anomalies or insufficient evidence. Alerts do not identify their cause or grant command authority.",
    stack: ["Rust"],
    summary: "Galadriel is an optional Rust monitor for possible sensor tampering and persistent measurement anomalies. It checks prediction residuals, changes over time, and agreement between sensor modalities. Faults, model errors, or tampering can cause an alert. An alert alone does not identify the cause. Its NCP adapter records assessments without command authority. The current single-Visual integration test correctly reports insufficient evidence; multisensor tampering qualification and field validation remain open.",
    status: "Research-source and local v1 integration candidate; record-only output, explicit abstention, and no field-validation claim.",
    schemaType: "SoftwareSourceCode",
    languages: ["Rust"],
    licenses: ["MIT", "Apache-2.0"],
  },
  ...
  },
  {
    ...
    desc: "Experimental mission authorization and deterministic policy. Haldir gating is excluded from the NCP local v1 simulation profile.",
    stack: ["Rust"],
    summary: "Haldir Gate researches mission authorization, signed controller intents, admission, deterministic policy, and decision receipts. Its 0.9 review remains NO_GO. It is outside the NCP local v1 simulation profile; gated requests must be rejected before endpoint preparation. Local NCP digests are not Haldir signatures. No production readiness or airworthiness is claimed.",
